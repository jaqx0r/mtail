...
func (r *Runtime) CompileAndRun(name string, input io.Reader) error {
	...
	r.logmappings[name] = map[uint16]struct{}{}

	for _, log := range obj.RelevantLogs {
		hash := sha256.Sum256([]byte(log))
		r.logmappings[name][binary.BigEndian.Uint16(hash[:2])] = struct{}{} // is 8 enough?
	}
...
type Runtime struct {
	...
	logmappings   map[string]map[uint16]struct{} // logmappings is a map of hashed log names against the programs
...
func New(lines <-chan *logline.LogLine, wg *sync.WaitGroup, programPath string, store *metrics.Store, options ...Option) (*Runtime, error) {
	...
	r := &Runtime{
		...
		logmappings:   make(map[string]map[uint16]struct{}),
